import { Currency, TransferEth, BroadcastWithdrawal, LedgerAccountService } from '@tatumio/api-client'
...
import { abstractBlockchainOffchain, OffchainBroadcastFunction } from '@tatumio/shared-blockchain-abstract'
 ...
 */
const sendOffChainTransaction = async (body: TransferEth, web3: EvmBasedWeb3, provider?: string) => {
  ...
  withdrawal.fee = new BigNumber(
    client.utils.fromWei(
      new BigNumber(withdrawal.gasLimit || gasLimit).multipliedBy(gasPrice).toString(),
      'ether',
    ),
  ).toString()
}
...
export const offchain = (args: {
  ...
  web3: EvmBasedWeb3
  broadcastFunction: OffchainBroadcastFunction
}) => {
  return {
    prepare: {},
    send: {},
